...
#define SYS_LOG_DOMAIN "dev/uwp360"
...
static struct wifi_data uwp360_wifi_data;
#define DEV_DATA(dev) \
	((struct wifi_data *)(dev)->driver_data)
...
#if 0
int cp_check_running(cp_runing_bit_t reg_bit, int wait_ms)
{
	int value = 0;
	int cnt = 0;

	do {
		value = sys_read32(CP_RUNNING_CHECK_CR);
		if (value &(1 << reg_bit) ){
			return 0;
		}
		cnt++;
		k_sleep(10000); /* the sleep unit is 1ms=1000us */
	} while(cnt <= wait_ms);
...
NET_BUF_POOL_DEFINE(uwp360_tx_pool, CONFIG_WIFI_WINC1500_BUF_CTR,
		    CONFIG_WIFI_WINC1500_MAX_PACKET_SIZE, 0, NULL);
NET_BUF_POOL_DEFINE(uwp360_rx_pool, CONFIG_WIFI_WINC1500_BUF_CTR,
		    CONFIG_WIFI_WINC1500_MAX_PACKET_SIZE, 0, NULL);

K_THREAD_STACK_MEMBER(uwp360_stack, CONFIG_WIFI_WINC1500_THREAD_STACK_SIZE);
struct k_thread uwp360_thread_data;
...
static void stack_stats(void)
{
	net_analyze_stack("WINC1500 stack",
			  K_THREAD_STACK_BUFFER(uwp360_stack),
			  K_THREAD_STACK_SIZEOF(uwp360_stack));
}
 ...
 */
static int uwp360_get(sa_family_t family,
			...
			struct net_context **context)
{
	...
	//sd = &wifi_data.socket_data[(int)(*context)->user_data];
 ...
 */
static int uwp360_bind(struct net_context *context,
			 ...
			 socklen_t addrlen)
{
	...
	/* FIXME atmel uwp360 don't support bind on null port */
...
#if 0
	if (k_sem_take(&wifi_data.socket_data[socket].wait_sem,
		       WINC1500_BIND_TIMEOUT)) {
		SYS_LOG_ERR("bind error timeout expired");
		return -ETIMEDOUT;
	}

	return wifi_data.socket_data[socket].ret_code;
#endif
	return 0;
}
 ...
 */
static int uwp360_listen(struct net_context *context, int backlog)
{
	SOCKET socket = (int)context->user_data;
	int ret;
 ...
 * to a peer host.
 */
static int uwp360_connect(struct net_context *context,
			    const struct sockaddr *addr,
			    socklen_t addrlen,
...
#if 0
	wifi_data.socket_data[socket].connect_cb = cb;
	wifi_data.socket_data[socket].connect_user_data = user_data;
	wifi_data.socket_data[socket].ret_code = 0;
#endif
...
#if 0
	if (timeout != 0 &&
	    k_sem_take(&wifi_data.socket_data[socket].wait_sem, timeout)) {
		return -ETIMEDOUT;
	}
 ...
 * being established.
 */
static int uwp360_accept(struct net_context *context,
			   ...
			   void *user_data)
{
	...
	//wifi_data.socket_data[socket].accept_cb = cb;
	...
	if (timeout) {
		...
		k_sem_take(&wifi_data.socket_data[socket].wait_sem,
			   K_FOREVER);
	}
 ...
 */
static int uwp360_send(struct net_pkt *pkt,
			 ...
			 void *user_data)
{
	...
	//wifi_data.socket_data[socket].send_cb = cb;
	//wifi_data.socket_data[socket].send_user_data = user_data;
 ...
 * This function is called when user wants to send data to peer host.
 */
static int uwp360_sendto(struct net_pkt *pkt,
 ...
 * host.
 */
static int uwp360_recv(struct net_context *context,
...
#if 0
	ret = prepare_pkt(&wifi_data.socket_data[socket]);
	if (ret) {
		SYS_LOG_ERR("Could not reserve packet buffer");
		return -ENOMEM;
	}

	wifi_data.socket_data[socket].recv_cb = cb;
	wifi_data.socket_data[socket].recv_user_data = user_data;

	ret = recv(socket, wifi_data.socket_data[socket].pkt_buf->data,
 ...
 */
static int uwp360_put(struct net_context *context)
{
	return 0;
}
...
static void handle_wifi_con_state_changed(void *pvMsg)
{
	...
	case M2M_WIFI_DISCONNECTED:
		...
		if (wifi_data.connecting) {
			wifi_mgmt_raise_connect_result_event(wifi_data.iface,
		...
		wifi_mgmt_raise_disconnect_result_event(wifi_data.iface, 0);
#endif
	...
	case M2M_WIFI_CONNECTED:
		...
		wifi_mgmt_raise_connect_result_event(wifi_data.iface, 0);
#endif
...
//	net_if_ipv4_addr_add(wifi_data.iface, &addr, NET_ADDR_DHCP, 0);
}

static void reset_scan_data(void)
{
	//wifi_data.scan_cb = NULL;
	//wifi_data.scan_result = 0;
}
...
static void handle_scan_result(void *pvMsg)
{
	...
	wifi_data.scan_cb(wifi_data.iface, 0, &result);

	k_yield();

out:
	if (wifi_data.scan_result < m2m_wifi_get_num_ap_found()) {
		m2m_wifi_req_scan_result(wifi_data.scan_result);
		wifi_data.scan_result++;
	} else {
		wifi_data.scan_cb(wifi_data.iface, 0, NULL);
		reset_scan_data();
	}
#endif
}
...
static void handle_scan_done(void *pvMsg)
{
	...
	if (pstrInfo->s8ScanState != M2M_SUCCESS) {
		wifi_data.scan_cb(wifi_data.iface, -EIO, NULL);
		reset_scan_data();
...
static void uwp360_wifi_cb(u8_t message_type, void *pvMsg)
{
...
static void handle_socket_msg_listen(struct socket_data *sd, void *pvMsg)
{
	...
	if (listen_msg->status) {
		SYS_LOG_ERR("uwp360_socket_cb:LISTEN: error %d %s",
			    listen_msg->status,
...
static void handle_socket_msg_accept(struct socket_data *sd, void *pvMsg)
{
	...
	if (sd->accept_cb) {
		...
		a_sd = &wifi_data.socket_data[accept_msg->sock];
...
static void uwp360_socket_cb(SOCKET sock, u8_t message, void *pvMsg)
{
	struct socket_data *sd = &wifi_data.socket_data[sock];
...
static void uwp360_thread(void)
{
	printk("%s %d.\n", __func__, __LINE__);
	while (1) {
...
static int uwp360_mgmt_scan(struct device *dev, scan_result_cb_t cb)
{
...
static int uwp360_mgmt_connect(struct device *dev,
...
static int uwp360_mgmt_disconnect(struct device *device)
{
	printk("%s %d.\n", __func__, __LINE__);
#if 0
...
static struct net_offload uwp360_offload = {
	.get		= uwp360_get,
	.bind		= uwp360_bind,
	.listen		= uwp360_listen,
	.connect	= uwp360_connect,
	.accept		= uwp360_accept,
	.send		= uwp360_send,
	.sendto		= uwp360_sendto,
	.recv		= uwp360_recv,
	.put		= uwp360_put,
};

static void uwp360_iface_init(struct net_if *iface)
{
	struct wifi_data *data=DEV_DATA(iface->if_dev->dev);
	...
	iface->if_dev->offload = &uwp360_offload;

	wifi_cmd_start_sta((u8_t *)data);

	data->iface = iface;
}

static const struct net_wifi_mgmt_offload uwp360_api = {
	.iface_api.init = uwp360_iface_init,
	.scan		= uwp360_mgmt_scan,
	.connect	= uwp360_mgmt_connect,
...
static int uwp360_init(struct device *dev)
{
	...
	SYS_LOG_DBG("UWP360 WIFI driver Initialized");

	return 0;
}

NET_DEVICE_OFFLOAD_INIT(uwp360, CONFIG_WIFI_UWP360_NAME,
			uwp360_init, &uwp360_wifi_data, NULL,
			CONFIG_WIFI_INIT_PRIORITY, &uwp360_api,
			1550);
