...
int eng_diag_encode7d7e(char *buf, int len, int *extra_len) {
  ...
  for (i = 0; i < len; i++) {
    if ((buf[i] == 0x7d) || (buf[i] == 0x7e)) {
      tmp = buf[i] ^ 0x20;
      printf("%s: tmp=%x, buf[%d]=%x", __FUNCTION__, tmp, i, buf[i]);
      buf[i] = 0x7d;
      for (j = len; j > i + 1; j--) {
        buf[j] = buf[j - 1];
      }
      ...
      printf("%s: AFTER:[%d]", __FUNCTION__, len);
      for (j = 0; j < len; j++) {
        printf("%x,", buf[j]);
      }
    }
...
int eng_atdiag_wifi_euthdlr(char *buf, int len, char *rsp, int module_index) {
  ...
  ENG_LOG("%s(), module=%d, buf = %s\n", __FUNCTION__, module_index, buf);
  tmp = strchr(buf, '?');
  if (tmp != NULL) {
  	printf("it is a query command\n");
	start = strchr(buf, '=');
	if (start == NULL) {
		printf("can not find = charator in buf\n");
		strcpy(rsp, "can not match the at command");
		return -1;
	}
	start++;
	strcpy(args0, start);
	printf("command name : args0 : %s \n", args0);
  } else {
  	printf("it is a set command\n");
  ...
  switch (cmd_index) {
    ...
    case WIFIANT_INDEX: {
      printf("%s(), case:WIFIANT_INDEX\n", __func__);
      wifi_ant_set(atoi(data[1]), rsp);
    } break;

    /* NETMODE */
    case WIFINETMODE_INDEX: {
      printf("%s(), case:WIFINETMODE_INDEX\n", __FUNCTION__);
  ...
  do {
    ...
    if (w_cnt < 0) {
		...
		printf("write ebusy\n");
        		...
        		printf("eng_vdiag_r no log data write:%d ,%s\n", w_cnt,
                	strerror(errno));
      	}
    }
    printf("write success, wcnt : %d\n",w_cnt);
    r_cnt -= w_cnt;
    offset += w_cnt;
    printf("check rcnt : %d\n", r_cnt);
  } while (r_cnt > 0);

	printf("write finished, return offset : %d\n",offset);
  return offset;
}
...
int eng_diag_apcmd_hdlr(unsigned char *buf, int len, char *rsp) {
  ...
  *(ptr + i - 1) = '\0';
  ...
  printf("%s: s_cmd_index: %d \n", __FUNCTION__, s_cmd_index);
  eng_linuxcmd_hdlr(s_cmd_index, ptr, rsp);

  rlen = strlen(rsp);

  printf("%s:rlen:%d; %s", __FUNCTION__, rlen, rsp);
...
int eng_diag_user_handle(int type, char *buf, int len)
{
  ...
  printf("%s: eng_diag_write2pc eng_diag_buf=%s;eng_diag_len:%d !\n",
  ...
  if (ret <= 0) {
    printf("%s: eng_diag_write2pc ret=%d !\n", __FUNCTION__, ret);
  }
...
int eng_diag(char *buf, int len)
{
  ...
  memset(eng_diag_buf, 0, sizeof(eng_diag_buf));
    ...
    if (ret_val) {
      printf("start to write OK test tool\n");
      eng_diag_buf[0] = 0x7e;
