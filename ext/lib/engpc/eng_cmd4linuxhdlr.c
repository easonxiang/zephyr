...
{
	printf("cmd is : %d  req is : %s\n", cmd, req);

	if (cmd >= (int)NUM_ELEMS(eng_linuxcmd)) {
		printf("%s: no handler for cmd%d", __FUNCTION__, cmd);
		return -1;
  	}
...
int eng_linuxcmd_bteutmode(char *req, char *rsp) {
	...
	printf("%s(), cmd = %s", __FUNCTION__, req);
