...
#define wifi_status_dump()                                             \
  do {                                                                 \
    printf("%s(), eut_enter = %d, tx_start = %d, rx_start = %d\n",    \
...
static int get_iwnpi_ret_line(const char *flag, const char *out_str, const int out_len) {
  ...
  if (NULL != flag && NULL == out_str) {
    printf("%s(), flag(%s) out str is NULL, return -100.\n", __FUNCTION__, flag);
  ...
  printf("leaving %s(), out_str = %s, ret = %d\n", __FUNCTION__, out_str, ret);
  return ret;
}
...
static int wifi_npi_start(char *rsp)
{
	...
	printf("just call npi start\n");
	if (0 != exec_iwnpi_cmd_with_status(argc, cmd)) {
		printf("exec iwnpi wlan0 start error");
		return -1;
	}
	return 0;
}
...
ok:
  ...
  printf("start to set eut_enter status\n");
...
err:
  printf("start to set eut_enter to zero\n");
...
int wifi_eut_set(int cmd, char *rsp) {
  printf("entry %s(), cmd = %d\n", __FUNCTION__, cmd);
  if (cmd == 1) {
    start_wifieut(rsp);
  } else if (cmd == 0) {
    stop_wifieut(rsp);
  } else {
    printf("%s(), cmd don't support", __func__);
    sprintf(rsp, "%s%s", EUT_WIFI_ERROR, "error");
  }
  return 0;
}
...
int wifi_channel_set_marlin3(char *ch_note1, char *ch_note2, char *rsp) {
  ...
  if (wifi_enter_eut_mode() == 0)
    ...
    if (primary_channel < 1 || center_channel < 1)
      goto err;
    printf("%s(): prim_chn = %d, cen_chn = %d.", __FUNCTION__, primary_channel, center_channel);
...
int wifi_band_set(wifi_band band, char *rsp) {
  ...
  if (WCN_HW_MARLIN3 == get_wcn_hw_type()) {
    printf("ADL entry %s(),marlin3 isn't support set band", __func__);
...
int wifi_bandwidth_set(wifi_bandwidth band_width, char *rsp) {
  ...
  printf("entry %s(), band_width = %d\n", __FUNCTION__, band_width);
  if (wifi_enter_eut_mode() == 0)
    goto err;

  if (band_width > WIFI_BANDWIDTH_160MHZ) {
    printf("%s(), band num is err, band_width = %d\n", __FUNCTION__, band_width);
    goto err;
  }
  ...
  if (0 != exec_iwnpi_cmd_with_status(argc, argv)) {
      printf("exec set_cbw command error\n");
      goto err;
  }
...
int wifi_tx_mode_set(wifi_tx_mode tx_mode, char *rsp) {
  ...
  printf("entry %s(), tx_mode = %d", __FUNCTION__, tx_mode);
  if (wifi_enter_eut_mode() == 0)
    goto err;

  if (tx_mode > WIFI_TX_MODE_LOCAL_LEAKAGE) {
    printf("%s(), tx_power's value is invalid, tx_mode = %d", __FUNCTION__,
            tx_mode);
    goto err;
  }
  ...
  if (WIFI_TX_MODE_CW == tx_mode) {
    printf("%s(), tx_mode is CW, tx_start = %d", __FUNCTION__,
            g_wifi_data.tx_start);
    if (1 == g_wifi_data.tx_start) {
      printf("tx start already on, need stop tx first\n");
      argv[2] = "tx_stop";

      if (0 != exec_iwnpi_cmd_with_status(argc, argv))
        goto err;

      printf("ADL %s(), set tx_start to 0", __func__);
      g_wifi_data.tx_start = 0;
    }
    printf("start to send sin_wave\n");
    argv[2] = "sin_wave";
    ...
    printf("%s(), set sin_wave_start is 1.", __FUNCTION__);

  } else { /* other mode, send value to CP2 */
    printf("%s(), tx_mode is other mode, mode = %d", __FUNCTION__, tx_mode);
    int cmd_cnt = 4;
    char *cmd_str[4];
    cmd_str[0] = "iwnpi";
    cmd_str[1] = "wlan0";
...
int wifi_netmode_set(int chain, char *rsp) {
  printf("marlin3 not support this command,so return ok\n");
  return return_ok(rsp);
