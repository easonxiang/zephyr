...
static void sblock_thread(void *p1, void *p2, void *p3)
{
    ...
    while(1)
    {
		ipc_info("wait for channel %d data.", sblock->channel);
...
/*这个中断处理是在main.c里面注册的block的中断处理 是通知应用处理sblock的消息的*/
        ...
        case SMSG_TYPE_CMD:
			...
			recovery = 1;
			ipc_info("ap cp create %d channel success!",sblock->channel);
...
//这里分配共享内存 在NuttX里面sblock的共享内存怎么来确定
	...
	ipc_info( "smem_addr 0x%x record_share_addr 0x%x",sblock->smem_addr,record_share_addr);
    ...
    ipc_info("0x%p ringhd %p poolhd %p",sblock,ringhd,poolhd);
...
/*这里是将每个blk的地址放到pool里面管理*/
	...
	ipc_info("r_txblks %p %p %p %p",ring->r_txblks,ring->r_rxblks,ring->p_txblks,ring->p_rxblks);
	...
	sblock->pid = k_thread_create(&sblock->thread,
			...
			(void *)sblock, NULL, NULL,
			 K_PRIO_COOP(7), 0, 0);
	if(sblock->pid == 0) {
		ipc_info("Failed to create kthread: sblock-%d-%d", dst, channel);
		return -1;
	}
		...
		int (*callback)(int ch, void *data,int len))
{
	int dst=0;
	struct sblock_mgr *sblock = &sblocks[dst][channel];
    ipc_info("%d channel=%d %p", dst, channel,callback);
...
void sblock_put(uint8_t dst, uint8_t channel, struct sblock *blk)
{
	...
	ipc_info("%d %d ",poolhd->txblk_rdptr,txpos);
...
int sblock_get(uint8_t dst, uint8_t channel, struct sblock *blk, int timeout)
{
	...
	if (poolhd->txblk_rdptr != poolhd->txblk_wrptr &&
		...
		ipc_info("txpos %d poolhd->txblk_rdptr %d",txpos,poolhd->txblk_rdptr);
...
static int sblock_send_ex(uint8_t dst, uint8_t channel,uint8_t prio, struct sblock *blk, bool yell)
{
	...
	if (sblock->state != SBLOCK_STATE_READY) {
		ipc_info("sblock-%d-%d not ready!", dst, channel);
	...
	ipc_info("txpos: %d, wrptr: 0x%x", txpos, ringhd->txblk_wrptr);
...
int sblock_receive(uint8_t dst, uint8_t channel, struct sblock *blk, int timeout)
{
	...
	ipc_info("sblock_receive: dst=%d, channel=%d, timeout=%d",dst, channel, timeout);
...
int sblock_release(uint8_t dst, uint8_t channel, struct sblock *blk)
{
	...
	poolhd->rxblk_wrptr = poolhd->rxblk_wrptr + 1;
	ipc_info("sblock_release: addr=%x", ring->p_rxblks[rxpos].addr);
