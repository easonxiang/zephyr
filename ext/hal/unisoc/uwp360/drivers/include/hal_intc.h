...
#define AON_INT_UART	12
	...
	static inline void uwp360_irq_enable(u32_t channel)
	{
		sci_reg_or(REG_IRQ_ENABLE, BIT(channel));
	}

	static inline void uwp360_irq_disable(u32_t channel)
	{
		sci_reg_or(REG_IRQ_DISABLE, BIT(channel));
	}

	static inline u32_t uwp360_irq_status(u32_t channel)
	{
		u32_t reg;
	...
	static inline void uwp360_irq_trigger_soft(void)
	{
		sci_write32(REG_IRQ_SOFT, BIT(INT_SOFT));
	}

	static inline void uwp360_irq_clear_soft(void)
	{
		sci_write32(REG_IRQ_SOFT, ~BIT(INT_SOFT));
	}

	static inline void uwp360_fiq_enable(u32_t channel)
	{
		sci_reg_or(REG_FIQ_ENABLE, BIT(channel));
	}

	static inline void uwp360_fiq_disable(u32_t channel)
	{
		sci_reg_or(REG_FIQ_DISABLE, BIT(channel));
	}

	static inline u32_t uwp360_fiq_status(u32_t channel)
	{
		u32_t reg;
	...
	static inline void uwp360_fiq_trigger_soft(void)
	{
		sci_write32(REG_FIQ_SOFT, BIT(INT_SOFT));
	}

	static inline void uwp360_fiq_clear_soft(void)
	{
		sci_write32(REG_FIQ_SOFT, ~BIT(INT_SOFT));
	}

	static inline void uwp360_aon_irq_enable(u32_t channel)
	{
		sci_reg_or(REG_AON_IRQ_ENABLE, BIT(channel));
	}

	static inline void uwp360_aon_irq_disable(u32_t channel)
	{
		sci_reg_or(REG_AON_IRQ_DISABLE, BIT(channel));
	}

	static inline u32_t uwp360_aon_irq_status(u32_t channel)
	{
		u32_t reg;
	...
	static inline void uwp360_aon_irq_trigger_soft(void)
	{
		sci_write32(REG_AON_IRQ_SOFT, BIT(INT_SOFT));
	}

	static inline void uwp360_aon_irq_clear_soft(void)
	{
		sci_write32(REG_AON_IRQ_SOFT, ~BIT(INT_SOFT));
	}
#ifdef __cplusplus
