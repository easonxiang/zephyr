...
#define EIC_MAX_NUM				(3)
	...
	static inline void uwp360_hal_eic_enable(u32_t num, u32_t channel)
	{
		sci_reg_or(eic_base[num] + EIC_LATCH_INTEN, BIT(channel));
	}

	static inline void uwp360_hal_eic_disable(u32_t num, u32_t channel)
	{
		sci_reg_and(eic_base[num] + EIC_LATCH_INTEN, ~BIT(channel));
	}

	static inline void uwp360_hal_eic_enable_sleep(u32_t num, u32_t channel)
	{
		sci_reg_and(eic_base[num] + EIC_LATCH_INTMODE, ~BIT(channel));
	}

	static inline void uwp360_hal_eic_disable_sleep(u32_t num, u32_t channel)
	{
		sci_reg_or(eic_base[num] + EIC_LATCH_INTMODE, BIT(channel));
	}

	static inline u32_t uwp360_hal_eic_status(u32_t num, u32_t channel)
	{
		u32_t reg;
	...
	static inline void uwp360_hal_eic_clear(u32_t num, u32_t channel)
	{
		sci_write32(eic_base[num] + EIC_LATCH_INTCLR, BIT(channel));
	}

	static inline void uwp360_hal_eic_set_trigger(u32_t num,
