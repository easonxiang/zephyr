...
#define REG_IPI_INT_CLR_LOCAL	(BASE_IPI + 0x8)
	...
	static inline void uwp360_hal_ipi_trigger(enum ipi_core core,
	...
	static inline void uwp360_hal_ipi_clear_remote(enum ipi_core core,
			enum ipi_int_type type)
	{
		if (core >= IPI_CORE_MAX_NUM) {
			printk("invalid ipi core number: %d.\n", core);
	...
	static inline void uwp360_hal_ipi_clear_local(enum ipi_core core,
			enum ipi_int_type type)
	{
		if (core >= IPI_CORE_MAX_NUM) {
			printk("invalid ipi core number: %d.\n", core);
